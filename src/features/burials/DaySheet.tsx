  ...
  return createPortal(
    ...
    >
      ...
      <div className="sticky top-0 z-10 flex items-center gap-2 border-b border-line bg-surface px-4 py-2.5 print:hidden">
        <span className="text-small-title font-display font-semibold text-ink">
    ...
    >
      <header className="flex items-end justify-between gap-4 border-b border-ink pb-3">
        <div>
          ...
          <h2 className="text-page-title font-display font-semibold leading-tight text-ink print:text-[27px]">
            {fmtDateLong(date)}
          </h2>
        </div>
        <div className="text-caption text-right leading-snug text-muted print:text-[11.5px]">
        ...
        return (
          <div key={slot} className="mt-5 border-b border-line-soft pb-5 last:border-b-0">
            <div className="flex items-baseline justify-between gap-3">
              <h3 className="text-caption font-semibold uppercase text-ink print:text-[12px]">{SLOT_LABEL[slot]}</h3>
              {interment && (
                <span className="text-caption text-muted print:text-[11.5px]">
            ...
            {!job || !interment ? (
              <p className="text-body mt-2 text-muted print:text-[13px]">
              ...
              <div className="mt-2 grid grid-cols-[92px_1fr] gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-small-title font-display font-semibold leading-tight text-ink print:text-[19px]">
                    {deceasedFullName(interment)}
                  </p>
                  <p className="text-body mt-0.5 text-muted print:text-[12.5px]">
                    Lot{' '}
                  ...
                  <ul className="mt-3 grid grid-cols-2 gap-x-6 gap-y-1.5">
                    {GROUNDS_CHECKLIST.map((c) => {
                      ...
                      return (
                        <li key={c.key} className="text-body flex items-center gap-2 print:text-[12.5px]">
                          <span
                            aria-hidden
                            className="text-micro grid size-5 shrink-0 place-items-center rounded-sm border border-ink leading-none text-ink print:size-[13px] print:text-[10px]"
                          >
                            {done ? '✓' : ''}
                          </span>
      ...
      <footer className="mt-7 grid grid-cols-2 gap-8">
        {['Grounds crew signature', 'Verified by'].map((label) => (
          <div key={label}>
            ...
            <p className="text-caption mt-1 flex items-center gap-1.5 text-muted print:text-[11px]">
