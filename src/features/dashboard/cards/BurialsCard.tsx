          ...
          {d.rows.map((r) => (
            <li key={r.id}>
              <button
                ...
                className="flex min-h-10 w-full flex-wrap items-center gap-2 rounded-md px-1.5 py-1 text-left transition-colors hover:bg-surface-2"
              >
                <span className="w-[46px] shrink-0 text-caption tabular text-muted">
                  {fmtDateShort(r.date)}
                </span>
                <span
                  className={cn(
                    'shrink-0 rounded px-1 py-px text-micro font-semibold uppercase tracking-[0.06em]',
                    r.slot === 'morning'
                ...
                {r.dayFull && (
                  <span className="shrink-0 rounded border border-line px-1 text-micro font-semibold uppercase tracking-[0.06em] text-muted">
                    Full
                  </span>
                )}
